...
if($data_login) {     // if the cookie does exsist
  ($GLOBALS["___mysqli_ston"] = mysqli_connect("localhost", "root", "root")) or die ("Whoops");  //connect to db
  ...
  $r = ((mysqli_query($GLOBALS["___mysqli_ston"], "USE hotelmis")) ? mysqli_query($GLOBALS["___mysqli_ston"], $sql) : false);  //execute sql

  if(!mysqli_num_rows($r)) {    // if there are no rows, means no matches for that username
    header("Location: index.php");   // so go back to the login page
  }

 	$chkusr = mysqli_fetch_array($r); //if we got passed the last part, then get the username/password set that match that username
